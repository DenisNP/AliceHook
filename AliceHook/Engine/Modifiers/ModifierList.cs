    ...
    public class ModifierList : ModifierBaseKeywords
    {
        ...
        protected override SimpleResponse Respond(AliceRequest request, State state)
        {
            ...
            if (state.User.Webhooks.Count == 0)
            {
                return new SimpleResponse
                {
                    Text = "У вас пока нет вебхуков. Самое время добавить. Можете посмотреть примеры сценариев.",
                    Tts = "У вас пока нет вэбх+уков. Самое время добавить. Можете посмотреть примеры сценариев.",
                    Buttons = state.User.Token.IsNullOrEmpty()
                        ? new []{ "Добавить вебхук", "Примеры", "Авторизация", "Помощь", "Выход" }
                        : new []{ "Добавить вебхук", "Примеры", "Помощь", "Выход" }
                };
            }
